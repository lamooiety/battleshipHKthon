...
def ShipLogic(round, yourMap, yourHp, enemyHp, p1ShotSeq, p1PrevHit, storage):
    ...
    if not p1PrevHit and (storage == [] or storage[0] == 1):
        ...
        if storage == []:
            storage.append(1)
            # create a heat map
            heat_map = {(x, y): random.random() for x in range(1, GRIDSIZE+1) for y in range(1, GRIDSIZE+1)}
        ...
        # otherwise, return by checkerbox
        blackboard = []
        whiteboard = []
    ...
    # test the crossboard until hit again
        ...
        if p1PrevHit:
            ...
            if len(storage[3]) >= 5:
                print("4")
                return p1ShotSeq[-1], [1, storage[1]]
            ...
            # followng rows
                ...
                if min == 1 or [min-1, prevY] in p1ShotSeq:
                    if max == 10 or [max+1, prevY] in p1ShotSeq:
                        # back to hunting
                        print("8")
                        return [max+1, prevY], [1, storage[1]]
                    else:
                        print("9")
